...
alias go-be="pushd ~/git/styleseat"
alias go-fe="pushd ~/git/mobileweb"
alias go-srch="pushd ~/git/search-api-2.0"
...
alias ss-stop-web="go-be && docker compose stop web && popd"
...
alias ss-up="go-be && ss-docker-login && docker compose up --build -d && docker compose logs -f web worker && popd"
alias ss-up-kibana="go-srch && ss-docker-login && docker-emu-reset && aws-vault exec styleseat-dev -- docker compose up --build -d kibana && popd"
# alias ss-search-up="go-srch && ss-up-kibana && docker-emu-x86_64 && aws-vault exec styleseat-dev -- docker compose up --build -d && docker compose logs -f && popd"
alias ss-search-up="go-srch && docker-emu-reset && docker-emu-x86_64 && aws-vault exec styleseat-dev -- docker compose up --build -d && docker compose logs -f && popd"
alias ss-up-debug="export DEBUGPY_ADDRESS=0.0.0.0:5679 && ss-up"
alias ss-web-debug="go-be && docker compose run --service-ports web sh -c 'pip install ipdb && python manage.py runserver 0.0.0.0:8000' && popd"
alias ss-test-vscode="go-be && ss-stop-web && docker compose run -e TEST_MODE=1 -e DEBUGPY_ADDRESS=0.0.0.0:5680 -e DEBUGPY_TEST_WAIT=1 -e DJANGO_SETTINGS_MODULE=settings.test -p 5680:5680 web sh -c 'echo \"ptw -v styleseat/schedule/tests/views/test_online_booking_payment_hold.py -- -s --sw\" && sh' && popd"
alias ss-migrate="go-be && docker compose run web python manage.py migrate --noinput && popd"
...
alias ss-email='echo "bdodd+$(date +%g%m%d%H%M)@styleseat.com" | pbcopy && echo "Email copied to clipboard"'
alias ss-phone='echo "555$(shuf -i 2000000-9999999 -n 1)" | pbcopy && echo "Phone number copied to clipboard"'
...
# Git Help
alias push_tags='echo "Push a tag to a remote: git pushd <remote> :refs/tags/<tag>"'
